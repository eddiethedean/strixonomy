...
**Pilot-ready for many OWL/OBO workflows in VS Code and CI** — not a full Protégé replacement for every profile. Use [What ships today](SHIPPED.md) and [Known limitations](known-limitations.md) for the capability matrix, [Production readiness](guides/production-readiness.md) for pilot vs production tiers, and [Protégé decision guide](guides/protege-decision.md) for gap analysis. Pin releases in CI (`--version 0.28.1`) and review [API stability](guides/api-stability.md) before embedding Rust libraries.

## Installation

**Which version should I install?**

Pin to the tagged release in [`docs/TAGGED_RELEASE`](https://github.com/eddiethedean/strixonomy/blob/main/docs/TAGGED_RELEASE) (currently **0.28.1**). See [Versions and channels](guides/versions-and-channels.md) for Marketplace vs GitHub Releases vs crates.io vs Read the Docs `latest`.
...
See [Install CLI & CI (detail)](install-cli-ci.md). Always pin with `--version 0.28.1` in CI so you do not surprise-upgrade.
...
- Set `strixonomy.lspPath` to a local `strixonomy-lsp` binary (`cargo install strixonomy-lsp --locked --version 0.28.1`) when debugging a custom build — trusted workspaces only.
