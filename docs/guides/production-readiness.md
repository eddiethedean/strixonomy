...
|-------|---------|---------|
| **Pre-1.0** | **0.26.x (latest tagged)** | Pin `cargo install strixonomy-cli --locked --version 0.26.2` in CI. Library APIs may change until [v1.0](../design/v1.0_BACKLOG.md). |
...
Pin CLI version in CI: release binary with `VERSION=0.26.2` or `cargo install strixonomy-cli --locked --version 0.26.2`.
