...
1. On a connected staging machine, download from [GitHub Releases](https://github.com/eddiethedean/strixonomy/releases) for version **v0.28.1** (or your pinned version):
   - `strixonomy-v0.28.1.vsix` (pattern: `strixonomy-v<version>.vsix`)
   - `SHA256SUMS`
   - `NOTICES`
...
|----------|----------------|
| Version pin | Standardize on one release tag (e.g. `v0.28.1`) across VSIX and CLI |
...
|----------|-------------------|--------------|
| Linux x64 | **Yes** | — |
| macOS | **No** | `cargo install strixonomy-cli --locked --version 0.28.1` (requires Rust on agent) |
