...
!!! tip "Prefer the tutorial folder"
    After [First success](../guides/first-success.md), point CLI commands at your **`strixonomy-tutorial`** directory (or unzipped [`strixonomy-tutorial.zip`](https://github.com/eddiethedean/strixonomy/releases/download/v0.28.1/strixonomy-tutorial.zip)). Paths like `fixtures/` exist **only in a git clone**. IDE-only users can skip CLI cookbooks.
